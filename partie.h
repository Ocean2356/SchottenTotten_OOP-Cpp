...
public:
    ...
    bool piocher(const Carte& carte) {
        ...
        cartes[x] = cartes[--nbCartes];  // déplacement de la dernière carte à l'emplacement de la carte pioche
        // il faut utiliser 'swap', et de plus vérifier le fonctionnement de & : on veut que la contenue réelle de l'objet carte soit piochée, pas la carte qui se trouve dans cartes[x]
        return true;
    }
