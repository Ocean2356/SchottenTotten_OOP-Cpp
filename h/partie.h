...
public:
    ...
    void ordonnerCartes(NumJoueur num_joueur);  // Méthode permettant de trier les cartes en fonction de la force d'un côté de la tuile
    ...
    virtual void placerCarte(const Carte& c, NumJoueur joueur_num){
        ...
        if (cotePlein(joueur_num) && !estPleine())  // le joueur vient de remplir la tuile de son côté et elle n'est pas pleine du côté de son adversaire
            rempli_en_premier = joueur_num;  // utilisé dans verif_revendiquable en cas d'égalité parfaite de combinaison
    }

    bool cotePlein(NumJoueur joueur_num) const{ return cartes_posees[(int) joueur_num].size() == nb_pleine;}  // Méthode vérifiant si le côté d'un joueur est plein
    virtual bool estPleine() const{ return cartes_posees[0].size() == nb_pleine && cartes_posees[1].size() == nb_pleine;}  // Méthode vérifiant si la tuile est pleine des deux côtés
    ...
    virtual void afficherCote(size_t cote) const{
        ...
        while (iter < 5){  // il y a au maximum 4 cartes d'un côté d'une tuile. Pour centrer l'affichage, on remplit toujours les positions vides
            cout << "    ";
            iter ++;
        }
        cout << "|";
    ...
    bool verifMemeCouleur(NumJoueur num_joueur) const;  // permet de vérifier que toutes les cartes d'un côté de la tuile ont la même couleur
    bool verifMemeForce(NumJoueur num_joueur) const;  // permet de vérifier que toutes les cartes d'un côté de la tuile ont la même force
    bool verifSuite(NumJoueur num_joueur);  // permet de vérifier que les cartes d'un côté de la tuile forment une suite
    unsigned int getSomme(NumJoueur num_joueur) const;  // permet de sommer la force de toutes les cartes d'un côté de la tuile
    virtual bool verifRevendiquable(NumJoueur num_joueur); // permet de vérifier qu'un joueur peut revendiquer une tuile
    virtual void revendiquer(NumJoueur num_joueur);  // revendique une tuile pour un joueur
...
public:
    ...
    unsigned int calculerScore(NumJoueur joueur_num) const;  // Méthode permettant de calculer le score qu'un joueur a obtenu en fin de partie
    JoueurGagnant estFinie() const;  // Méthode permettant de retourner le joueur gagnant ou JoueurGagnant::aucun si aucun joueur n'a gagné à ce stade
    ~Frontiere() = default;
private:
...
public:
    ...
    Movement choisirCarteAJouer(const Frontiere& frontiere, NumJoueur joueur_num);  // Méthode permettant la saisie par l'utilisateur d'une carte à jouer
    Movement choisirBornesARevendiquer(Frontiere& frontiere, NumJoueur joueur_num);  // Méthode permettant la saisie par l'utilisateur d'une ou plusieurs bornes à revendiquer
...
class Premiere : public Partie{  // classe abstraite permettant de spécifier certaines méthodes et caractéristiques communes aux variantes d'une partie de la première édition
public:
    void commencer(Ordre ordre) final;
    void agentSuivant();  // Méthode permettant de mettre à jour l'agent actif
...
protected:
    ...
    static const int NCARTENORMALE = NCOULEUR * NFORCE; // 9*6 = 54 cartes normales pour la première édition (auxquelles s'ajoutent d'éventuelles cartes tactiques)
...
class PremiereNormale final : public Premiere{  // classe correspondant à la variante Normale de la première édition de Schotten-Totten
public:
    PremiereNormale();
    void jouerTour();  // fonction permettant de jouer un tour dans son intégralité (choix de la carte à jouer, revendiquer une ou plusieurs bornes, piocher)
...
private:
    ...
    void initierAgents(Ordre ordre) override;  // Méthode permettant d'initialiser les agents (appelée par la méthode Premiere::commencer)
    void initierMains() override;  // Méthode permettant d'initialiser les mains (appelée par la méthode Premiere::commencer)
};
